...
public class LogarithmicLeveledScaleCamera extends Camera {
	private static final double SCALE_INCRIMENT = 0.1;
	private double[] scaleLevels = new double[50];
	private int currentScaleLevel;
	
	public LogarithmicLeveledScaleCamera() {
		super();
		for (int i = 0; i < this.scaleLevels.length; i++) {
			this.scaleLevels[i] = SCALE_INCRIMENT * (i + 1);
		}
